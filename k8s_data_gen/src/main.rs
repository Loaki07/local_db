...
fn print_usage() {
    ...
    println!("    Ingests a JSON file into OpenObserve");
    ...
    println!("  Logs/CPU:     avg(cpu_millicores) custom SQL with histogram");
    println!("  Logs/Errors:  count(*) filter log_level=ERROR");
    println!("  Metrics/CPU:  avg(cpu_percent) filter mode");
    println!("  Metrics/Mem:  avg(memory_percent) filter mode");
    println!("  Traces/P99:   avg(duration_ms) filter mode");
    println!("  Traces/Errs:  count(*) filter status=ERROR");
